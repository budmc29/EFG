  ...
  describe "#show" do
    context "as a lender user" do
      ...
      it "filters loans by scheme" do
        FactoryGirl.create(:loan, :completed, :legacy_sflg,
                           lender: current_lender, business_name: "Bar")
        FactoryGirl.create(:loan, :completed, :sflg,
                           lender: current_lender, business_name: "Woot")
      ...
      it "can export loan data as CSV" do
        ...
        expect(page.current_url).to eq(
          loan_state_url(id: "completed", format: "csv"))
      end
      ...
      it "exports loan data as a CSV with a scheme filter" do
        ...
        expect(page.current_url).to eq(
          loan_state_url(id: "completed", format: "csv", scheme: "efg"))
      end

      it "does not show the lender name" do
        dispatch(id: "completed")

        expect(page).not_to have_text("Lender")
        expect(page).not_to have_text(current_lender.name)
      end
    end
    ...
    context "as any other user" do
      ...
      it "displays the lender name" do
        ...
        expect(page).to have_text("Lender")
        expect(page).to have_text(current_lender.name)
      end
    end

    context "as any other user when the loan is from a sub lender" do
      let(:current_lender) { FactoryGirl.create(:lender) }
      let(:current_user) { FactoryGirl.create(:cfe_user) }
      ...
      before do
        ...
        FactoryGirl.create(:loan, :completed,
                           lender: current_lender,
                           business_name: "ACME",
                           sub_lender: "Important Sublender")
        FactoryGirl.create(:loan, :completed,
                           lender: current_lender,
                           business_name: "Foo")
      end
      ...
      it "displays the lender name" do
        ...
        expect(page).to have_text("Important Sublender")
        expect(page).to have_text(current_lender.name)
      end
    end
  end
